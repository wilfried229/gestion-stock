<div class="row">
    ...
    <div class="form-group col-lg-6 col-md-6 col-sm-6">
        <label for="role" class="col-form-label text-sm-right">Role</label>
        <select name="role" id="role" class="form-control custom-select2" required style="width: 100%; height: 38px;">
            <option selected disabled value="">Selectionnez</option>
            @foreach(array_keys(getUsersRoles()) as $role)
                <option @if (old('role',$user->role) == $role) selected @endif value="{{$role}}">{{getUsersRoles()[$role]}}</option>
            @endforeach
        </select>
    </div>

      <div class="form-group @if ($user->password ==null) col-lg-6 col-md-6 col-sm-12 @else col-sm-12 col-md-12 col-lg-12 @endif">
        <label for="password" class="col-form-label text-sm-right">Mot de passe</label>
     ...
     @endif
        ...
        <div class="form-group col-md-6 col-lg-6 col-sm-12">
            <label for="password_confirm" class="col-form-label text-sm-right">Confirmer le mot de passe</label>
        ...
        <div hidden id="radio_id" class="form-group col-lg-12 col-md-12 col-sm-12">
            <label for="radio_id" class="col-form-label text-sm-right">Station radio</label>
            <select name="radio_id" id="radio_id" class="form-control custom-select2" required style="width: 100%; height: 38px;">
                <option selected disabled value="">Selectionnez</option>
                @foreach(getRadios() as $radio)
                    <option @if (old('radio',$radio->id) =='0') selected @endif value="{{$radio->id}}">{{$radio->nom}}</option>
                @endforeach
            </select>
        </div>
